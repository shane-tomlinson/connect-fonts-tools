...
function generateSubsets(targetPath, argv, metaInfo, done) {
  ...
  var allSubsets = SUBSETS;
  metaInfo.subsets = [].concat(allSubsets);
  ...
  function processNext() {
    ...
    gen_subsets.subset(allSubsets, fontInfo.path, fontTargetPath, true, function(err) {
      if (err) return done(err);
      processNext();
    });
  }
...
function generateWebFontsFromTtf(targetPath, argv, metaInfo, done) {
  // first, get all font paths for all subsets.
  metaInfo.subsets = metaInfo.subsets || [];
  var fontPaths = Object.keys(metaInfo.fonts).map(function(fontName) {
    var defaultPath = metaInfo.fonts[fontName].path;
    var subsetPaths = metaInfo.subsets.map(function(subset) {
      return path.join(getFontTargetPath(targetPath), subset,
                path.basename(defaultPath));
    });

    return [defaultPath].concat(subsetPaths);
  }).reduce(function(currVal, prevVal) {
