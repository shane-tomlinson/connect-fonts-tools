...
function getLicenseText(licenseUrl, done) {
  request(licenseUrl, function(err, resp, body) {
    if (resp && resp.statusCode === 200) return done(null, body);

    done(null, "License available at " + licenseUrl);
  });
}
...
function generatePackageJson(targetPath, argv, metaInfo, done) {
  gen_package_json.write({
    ...
    license: {
      // make sure there are no line breaks in the description or else it
      // messes up the JSON output
      abbreviation: (metaInfo.common.license_desc || '').replace(/[\r\n]+/g, ' '),
      url: metaInfo.common.url_license
    },
    author: {
      name: argv.an || metaInfo.common.designer,
