  ...
  rimraf(targetPath, function(err) {
    ...
    mkdirp(targetPath, function(err) {
      ...
      function filter(name) {
        ...
        var copy = (name === sourcePath || ext === '.ttf' || ext === '.otf');
        if (!copy) { console.warn(path.basename(name) + ' is not a .otf or .ttf, skipping'); }
        return copy;
      }
  ...
  if (fonts.length > 1) {
    ...
    if (!regularFont) {
      ...
      if (regularFont) {
        ...
        var ext = path.extname(fontConfig.path);
        var newPath = path.join(path.dirname(fontConfig.path), newFontName + ext);
