...
function generateWebFontsFromTtf(targetPath, argv, metaInfo, done) {
  // First, build a list of all .ttf paths for all subsets.
  // Then generate web fonts for each ttf.
  metaInfo.subsets = metaInfo.subsets || [];
  var fontPaths = Object.keys(metaInfo.fonts).reduce(
      function(prevVal, fontName) {
    var fontPaths = metaInfo.fonts[fontName].paths;

    return prevVal.concat(fontPaths);
  }, []);
