...
module.exports = function(directoryName, done) {
  ls(directoryName, /\.(ttf|otf)$/i, true, function(err, files) {
    ...
    getTables(files, function(err, tables) {
      ...
      getCommonInfo(tables.name, function(err, commonInfo) {
        ...
        getFontInfo(tables, function(err, fontInfo) {
          ...
          var license = licenses.toIdentifier(commonInfo.license_desc, commonInfo.copyright, commonInfo.url_license);
          if (license) {
            commonInfo.license = license;
            commonInfo.license_desc = license.name + " " + license.version;
            commonInfo.url_license = license.url;
            commonInfo.license_abbreviation = license.abbreviation;
          }
...
function getFontInfo(tables, done) {
  ...
  tables.file.forEach(function(fileInfo, index) {
    ...
    fontInfo[fileInfo.basename] = {
      ...
      local: [ nameTable.font_full_name, nameTable.postscript_name ],
      path: fileTable.path
    };
  });
