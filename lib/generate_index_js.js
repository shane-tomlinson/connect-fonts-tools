 ...
 * config:
 *  target_dir
 *  meta_info.font.names
 *  meta_info.font.family
 *  meta_info.font.local
 */
exports.write = function(iConfig, done) {
  var outputConfig = getOutputConfig(deepCopy(iConfig));
  saveIndex(iConfig.target_dir, outputConfig, done);
};
...
function saveIndex(targetPath, outputConfig, done) {
  ...
  try {
    ...
    // check to make sure the index file loads correctly.
...
function getOutputConfig(config) {
  ...
  var outputConfig = {
    package: toString(config.package),
    author: toString(config.author),
    license: toString(config.license),
    font_common: toString(config.font),
    fonts: metaInfo.common ? getFontConfigFromTtfInfo(metaInfo)
                : getFontConfigFromFontname(config),
    aliases: getAliases(metaInfo)
  };
...
function toString(config) {
  return typeof config !== "undefined" ? JSON.stringify(config, null, 4).replace(/^}/gm, '  }') : "";
}
